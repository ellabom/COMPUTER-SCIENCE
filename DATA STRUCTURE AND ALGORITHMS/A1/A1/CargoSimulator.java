//EMMANUELLA EYO    CMPT 280        11291003        EEE917
...
public class CargoSimulator {
    ...
    public static void main(String args[]) {
        ...
        LinkedIterator280<Ship> shipIterator = sim.fleet.iterator();
        Grain gType = shipIterator.item().cargo.iterator().item().getType();
        while(shipIterator.itemExists()){
            if(gType == Grain.WHEAT){
                int numGrains = shipIterator.item().sacksOfGrainType(gType);
                System.out.println(shipIterator.item().getName() + " is carrying " + numGrains +
                        " sacks of wheat.");
            }
            shipIterator.goForth();
        }

        // TODO: Print out a message for each ship that is overloaded.
        shipIterator.goFirst();
        while(shipIterator.itemExists()){
            if(shipIterator.item().isOverloaded()){
                System.out.println(shipIterator.item().getName() + " is overloaded!");
            }
            shipIterator.goForth();
        }
