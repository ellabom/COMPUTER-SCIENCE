//EMMANUELLA EYO    CMPT-280    EEE917  11291003
...
public class Ship {
    ...
    public boolean isOverloaded() {
        ...
        LinkedIterator280<Sack> sackIterator = cargo.iterator();
        double total_weight = 0;
        while (sackIterator.itemExists())
        {
            total_weight += sackIterator.item().weight;
            sackIterator.goForth();
        }

        return total_weight > this.capacity;
    ...
    public static void main(String[] args){
        Ship ship = new Ship(ShipNames[1], 100);
        ship.loadSack(new Sack(Grain.WHEAT,55.0 ));
        ship.loadSack(new Sack(Grain.WHEAT, 65.0));
        ship.loadSack(new Sack(Grain.WHEAT, 29.2));
        ship.loadSack(new Sack(Grain.BARLEY, 12.8));
        ship.loadSack(new Sack(Grain.BARLEY, 33.99));
        ship.loadSack(new Sack(Grain.OATS, 30.0));

        int answer = ship.sacksOfGrainType(Grain.WHEAT);

        System.out.println(ship.isOverloaded());
