...
class InformedProblemV1(InformedProblem):
    ...
    def calc_h(self, s):
        """
        h(n) = unfilled area - area of the biggest block for that state.action
        """

        area_used = 0
        for row in range(self.gridsize, 0, -1):
            for col in range(self.gridsize):
                if s.heights[col] >= row:
                    area_used += 1

        remaining_area_of_grid = (s.gridsize * s.gridsize) - area_used

        b_dimensions = [b for b in s.blocks if s.blocks[b] > 0]
        biggest = max(b for b in b_dimensions if b[0] * b[0] > 0)

        hval = remaining_area_of_grid - (biggest[0] * biggest[1])
        return hval
...
class InformedProblemV2(InformedProblem):
    ...
    def calc_h(self, s):
        ...
        Calculate the number of unfilled spaces in the tetris grid
        :param s:the state of the tetris grid
        :return: the number of unfilled spces in the grid
        """

        # calculate the number of unfilled spaces with are represented by dots (.)
        # return the total number of unfilled spaces with count()

        state = str(s)  # convert state to a mutable string
        return state.count('.')
